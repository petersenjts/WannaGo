...
export default async function PartnersPage({
  ...
  return (
    <div>
      <h1 className="font-serif text-3xl text-ink">Partners</h1>
      <p className="mt-1 text-ink-soft">Hotels and restaurants you work with — contact info and trial status.</p>
      ...
      <div className="mt-8 overflow-x-auto rounded-xl border border-line bg-card">
        <table className="w-full min-w-[700px] text-left text-sm">
          ...
          <tbody>
            {partners.map((p) => (
              <tr key={p.id} className="border-b border-line last:border-0 hover:bg-paper-alt/50">
                ...
                <td className="px-4 py-3 align-top text-ink-soft">{p.city}</td>
                <td className="px-4 py-3 align-top text-ink-soft">{p.contactInfo ?? "—"}</td>
                <td className="px-4 py-3 align-top">
      ...
      <section className="mt-10 max-w-2xl rounded-xl border border-line bg-card p-6">
        <h2 className="font-serif text-xl text-ink">Add a partner</h2>
        <form action={createPartner} className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
